...
static void VescCan_SendInt32Packet(uint8_t packet_id, uint8_t controller_id, int32_t value, uint8_t dlc)
{
    CanTxMsg tx;
    uint32_t ext_id = ((uint32_t)packet_id << 8) | controller_id;
    uint8_t mailbox;
    uint8_t tx_status;
    ...
    tx.RTR = CAN_RTR_Data;
    tx.DLC = dlc;
    tx.Data[0] = (uint8_t)((value >> 24) & 0xFF);
    tx.Data[1] = (uint8_t)((value >> 16) & 0xFF);
    tx.Data[2] = (uint8_t)((value >> 8) & 0xFF);
    tx.Data[3] = (uint8_t)(value & 0xFF);
    tx.Data[4] = 0;
    tx.Data[5] = 0;
    tx.Data[6] = 0;
    tx.Data[7] = 0;
...
{
    VescCan_SendInt32Packet((uint8_t)VESC_CAN_PACKET_SET_RPM, controller_id, erpm, 8U);
...
{
    VescCan_SendInt32Packet((uint8_t)VESC_CAN_PACKET_SET_CURRENT_BRAKE, controller_id, current_mA, 4U);
}
